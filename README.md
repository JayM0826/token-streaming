...
The public technical-beta marketplace lives in [`apps/web`](apps/web). It uses platform sign-in, tenant-scoped D1 persistence, administrator authorization review, durable capacity offers, an encrypted authorized-gateway adapter, idempotent inference jobs, append-only usage/ledger records, and explicit standard/strict privacy modes. Prompt bodies are not persisted; replayable content, credentials, files, and keyed digest commitments have separate secrets and record-bound cryptographic contexts. Content views and purge operations are buyer-only, while the supplier dashboard receives aggregate business metrics. Artifact purge atomically queues deletion and moves each exact immutable chunk generation out of `ready` before R2 retry, so a stale live-generation filter cannot strand the queued delete. Cleanup transitions are deliberately bounded to one artifact and four generations per invocation, with four deletion-queue keys per round; a maximum 64-generation file can require 16 retryable invocations and is marked `deleted` only after every generation metadata row is gone. These small transitions do not make the complete maintenance request fit the lower D1 ceiling: production maintenance includes schema bootstrap and requires the Workers Paid 1,000-query-per-invocation quota as a hard deployment prerequisite. Independent maintenance validates four distinct 256-bit key domains before cleanup and reports object-deletion, unclaimed-expired-artifact, and pending-generation-tombstone backlogs plus their 24-hour breaches to scheduled monitoring. For artifact tasks, `content_purged_at` may mean only that an expired output was erased; full purge requires that marker together with instruction/output erasure, a purged and `deleted` artifact, and zero remaining generations, so 24-hour output expiry never hides the 48-hour input purge action. Once full artifact or task purge is true, every later explicit retry idempotently backfills any missing completion audit using a stable action/resource ID without duplicating an existing event. Browser code never receives provider credentials or chooses tenant identity; customer inference remains separate from the engineering Codex session. Promotional beta balances are durable test ledger entries but are not cash, and payment/KYC evidence/payout providers remain external requirements for commercial launch.

The deployable supplier runtime lives in [`apps/supplier-node`](apps/supplier-node). Individuals and organizations can run the same signed `gongsuanyun.gateway.v3` HTTPS node behind a public TLS proxy. The node keeps upstream credentials local, permits only exact configured models/data classes/provider hosts, and requires a non-empty durable replay-journal path; it refuses startup instead of degrading to memory-only protection. Restart-safe replay claims are fsynced journal-v2 records whose request bodies are represented only by gateway-token-keyed domain-separated HMAC commitments. Live v1 records migrate at startup; atomic bounded compaction and readiness both fail closed on storage faults. The current journal supports one active writer per gateway token; active-active HA requires a shared atomic replay-claim store, not merely a shared volume. The node also enforces request signatures, idempotency, body/response bounds, and RPM/TPM/concurrency limits, and never executes local Codex, tools, shell commands, or buyer file access. Public readiness exposes no inventory; approval requires a signed one-time challenge whose Provider, exact models, P0/P1 scope, and capacity must contain the requested authorization. Every successful inference also carries a node-signed execution evidence record binding the purchased Provider/model, actual upstream response model, input/output digests, usage, and provider request ID. The control plane writes that evidence and the balanced settlement atomically; any mismatch fails without billing.

The cross-platform supplier client lives in [`apps/supplier-agent`](apps/supplier-agent). It wraps the same headless node kernel with a loopback-only responsive GUI and safe CLI diagnostics for Windows, macOS, and Linux. First-run setup creates a profile-bound AES-256-GCM credential vault derived from a user passphrase and refuses overwrite. A one-time URL-fragment launch secret is immediately erased and exchanged for the HttpOnly loopback session Cookie. Version 0.3 reauthenticates and rate-limits every gateway-token reveal and speaks privacy-aware `artifact-worker.v2` for encrypted, resumable text-file tasks up to 256 MiB. Local checkpoint v2 has authenticated timestamps, a non-sliding six-hour maximum per attempt, bounded cleanup, and fresh-attempt reset. A resumed assignment with `resume_from_segment > 0` must have an authenticated checkpoint whose completed segment matches exactly or fail with `ARTIFACT_CHECKPOINT_REQUIRED` before any Provider call. If checkpoint deletion fails after accepted completion or any non-retryable terminal failure, the current process records a pending terminal delete and returns `CHECKPOINT_CLEANUP_FAILED`; it retries that delete before, and blocks, every later claim. After a restart, the authenticated fixed expiry still bounds checkpoint validity to six hours and pre-claim cleanup fails closed if physical deletion remains impossible. A stable public HTTPS reverse proxy or named outbound tunnel remains required for real-time short requests; the outbound file worker does not add an inbound port, and the Agent never changes router or firewall settings automatically.
...
For the Web deployment, a passing repository gate is not enough: production must run on Cloudflare Workers Paid with the 1,000 D1 queries-per-invocation quota before the maintenance workflow is enabled. Full maintenance calls `ensureSchema()` as well as bounded cleanup and is not supported under the lower query ceiling.
...
Set these variables in the same terminal that launches the CLI. Do not write API keys into `.ai/`, source files, or command history. Repository ignore rules cover local `.env`/`.env.*`, `.dev.vars`/`.dev.vars.*`, and `.wrangler/` state while retaining checked-in `.env.example` templates; ignored files are still local secrets and must not be copied into commits or artifacts.
