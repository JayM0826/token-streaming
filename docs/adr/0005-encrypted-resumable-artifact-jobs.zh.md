...
Supplier Agent 通过出站 HTTPS 每五秒发送签名 claim，同时写入两分钟有效的能力心跳。调度只选择满足以下全部条件的报价：供应主体与报价有效、授权令牌摘要匹配、Provider 和精确模型匹配、授权 ID 匹配、Agent 最近在线、媒体类型与文件大小在 Agent 声明范围内。任务领取后获得五分钟租约；检查点只能单调增加，租约到期可由同一 worker 从本地加密检查点恢复，最多三次。任务排队超过 30 分钟失败并释放预算。
