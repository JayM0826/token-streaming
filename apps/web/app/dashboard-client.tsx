      ...
      {modal === "authorization" && snapshot.supplier && (
        <Modal title="提交容量授权" kicker="PROVIDER AUTHORIZATION" onClose={() => setModal(null)} wide>
          <form onSubmit={(event) => {
            ...
            <label>授权证据引用<input name="evidenceRef" placeholder="contract-2026-001" required pattern="[A-Za-z0-9][A-Za-z0-9._:-]{2,127}" /></label>
...
function SupplierWorkspace({ snapshot, busy, onOpen, onToggleSupply }: { snapshot: MarketplaceDashboardSnapshot; busy: boolean; onOpen: (modal: ModalName) => void; onToggleSupply: () => void }) {
  ...
  return <section className="workspace-section"><div className="workspace-heading"><div><span className="section-kicker">SUPPLIER CONTROL PLANE</span><h1>供给管理</h1><p>{supplier.displayName} · {statusLabel(supplier.status)} · {supplier.countryCode}</p></div><div className="heading-actions"><button className="secondary-button" onClick={() => onOpen("authorization")}>提交授权</button><button className="primary-button" disabled={!approved} onClick={() => onOpen("offer")}>发布报价</button></div></div>
    <article className="panel agent-card"><div><span className="section-kicker">SUPPLIER AGENT</span><h2>先运行供应客户端，再提交节点授权</h2><p>客户端支持 Windows、macOS 和 Linux：本机加密保存 Provider Key，管理端口只绑定 127.0.0.1，并复用 v3 节点内核生成价服一致凭证。</p></div><ol><li><span>1</span>安装并打开 Supplier Agent</li><li><span>2</span>配置精确模型与稳定 HTTPS 地址</li><li><span>3</span>复制网关地址和令牌到授权申请</li></ol><div className="agent-note">封闭试运营安装包由平台管理员发放；不要提交 Provider API Key、Cookie 或个人订阅登录态。</div></article>
