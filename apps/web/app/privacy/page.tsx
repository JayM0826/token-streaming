...
export default function PrivacyPage() {
  return <main className="privacy-page">
    ...
    <section className="privacy-document">
      ...
      <article><span>05</span><div><h2>主动清除与不可变记录</h2><p>购买方可在控制台立即清除普通结果，或对文件任务发起两阶段取消与内容清除：尚未领取的任务立即终止；已有租约的任务先进入取消中，平台拒绝新的分块读取和结果结算，待 Agent 确认、租约过期或绝对期限到达后再物理删除 R2 对象。处理指令与结果会被清空；Agent 的加密检查点在终态时删除，并有独立的六小时硬过期清扫。为防止账务篡改，金额、用量和不含正文的执行凭证不会被覆盖或删除。</p></div></article>
      <article><span>06</span><div><h2>当前不接收的数据</h2><p>公开技术 Beta 只接受 P0 公开数据与 P1 一般业务数据。不要提交密码、API Key、Cookie、身份证件、医疗信息、金融账户、精确位置、商业绝密或其他 P2/P3 数据。文件内容不会被执行、解压，也不会触发其中的链接或工具指令。</p></div></article>
    </section>
