...
`marketplace-api.ts` defines the versioned browser/control-plane DTOs and stable error codes. `REVIEWER_CONFLICT` is the additive v1 refusal for an administrator attempting to review a supplier authorization from the same tenant; clients must not reinterpret it as an authentication failure. Views intentionally exclude gateway endpoints, encrypted tokens, crypto key ids, actor IDs, tenant IDs, and raw evidence documents.

Authorization lifecycle DTOs expose `pending`, `approved`, `rejected`, `withdrawn`, `revoked`, and the derived `expired` projection plus a monotonic authorization revision. Suppliers may withdraw a pending request, revoke a current approval, or submit a replacement Gateway token for live attestation. Token replacement is write-only: neither old nor replacement token is ever part of a response. State/CAS failures use `AUTHORIZATION_STATE_CONFLICT`; replacement-token reuse or generation conflicts use `GATEWAY_CREDENTIAL_CONFLICT`. A synchronous inference reservation that loses a credential-revision race becomes failed with the stable job error `GATEWAY_CREDENTIAL_ROTATED`; work already in `running` is not retroactively cancelled.
