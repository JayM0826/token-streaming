...
Authentication remains outside core. Each provider owns its native wire/process protocol while core consumes one `ModelProvider` interface. Network and model probes are opt-in, time-bounded, safely diagnosed, and separated from offline checks. Codex is the default provider and runs in an ephemeral read-only sandbox; explicit `auto` remains API-only. Provider output cannot bypass structured patch, permission, checkpoint, or verification boundaries.
