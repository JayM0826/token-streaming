...
test("buildRuntimePrompt includes handoff discipline for summary tasks", () => {
  ...
  assert.match(prompt.user, /orchestrator -> research: execution plan/);
  assert.match(prompt.user, /research -> final: repository context brief/);
});
...
function createPromptInput({ wantsPatch }) {
  const phases = wantsPatch
    ? [
        ...
        { id: "research", role: "research", title: "Research repository context", description: "", required: true },
  ...
  return {
    ...
    plan: {
      ...
      handoffs: wantsPatch
        ? [
            { from: "orchestrator", to: "research", artifact: "execution plan", description: "" },
            { from: "research", to: "coder", artifact: "repository context brief", description: "" },
            { from: "coder", to: "tester", artifact: "structured patch proposal", description: "" },
            { from: "tester", artifact: "verification result", description: "" }
          ]
        : [
            { from: "orchestrator", to: "research", artifact: "execution plan", description: "" },
            { from: "research", artifact: "repository context brief", description: "" }
          ],
      testCommands: [],
      notes: []
    },
    context: {
      overview: renderOverview(
        wantsPatch
          ? [
              { from: "orchestrator", to: "research", artifact: "execution plan" },
              { from: "research", to: "coder", artifact: "repository context brief" },
              { from: "coder", to: "tester", artifact: "structured patch proposal" },
              { from: "tester", artifact: "verification result" }
            ]
          : [
              { from: "orchestrator", to: "research", artifact: "execution plan" },
              { from: "research", artifact: "repository context brief" }
            ]
      ),
      relevantModules: [],
      relevantWorkflows: [],
